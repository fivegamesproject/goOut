...
const onFocus=()=>{interactiveInput.removeEventListener('focus',onFocus);interactiveInput.value=realInput.value;interactiveInput.type='number';interactiveInput.style.cursor='auto'}
const onBlur=()=>{realInput.value=interactiveInput.value;interactiveInput.type='text';try{interactiveInput.value=Number(realInput.value).toLocaleString(locale,formatOptions)}catch(error){interactiveInput.value=error.message}
interactiveInput.style.cursor='pointer';interactiveInput.addEventListener('focus',onFocus);if("createEvent" in document){var evt=document.createEvent("HTMLEvents");evt.initEvent("change",!1,!0);realInput.dispatchEvent(evt)}else{realInput.fireEvent("onchange")}}
onBlur();interactiveInput.addEventListener('blur',onBlur)}
